...
async function readGlobalStats() {
  const rows = await supabaseRest('global_stats?select=total_calls,model_counts,updated_at&id=eq.1&limit=1');
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const rows = await supabaseRpc('increment_global_call', { match_model_id: modelId });
      const row = rows && rows[0] ? rows[0] : null;
      return res.status(200).json({ stats: normalizeStatsRow(row) });
    }
