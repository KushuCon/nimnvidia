...
export default async function handler(req, res) {
  try {
    ...
    // Use permanent public URL — no expiry, no silent 403 after 30 days.
    // Previously used signed URLs (expiresIn: 30 days) which broke old messages silently.
    // Requires the bucket to have public read enabled in Supabase dashboard:
    //   Storage > nim-attachments > Policies > Allow public read
    const accessUrl = `${url}/storage/v1/object/public/${bucket}/${encodePath(objectPath)}`;
